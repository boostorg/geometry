...
struct geojson_visitor : public boost::geometry::detail::overlay::overlay_null_visitor
{
    ...
    void visit_turns(int phase, Turns const& turns)
    {
        ...
        for (auto const& turn : turns)
        {
            ...
            auto as_bool = [](bool b) { return b ? "true" : "false"; };

            m_writer.feature(turn.point);
            m_writer.add_property("index", index);
            m_writer.add_property("method", method());
            m_writer.add_property("cluster_id", turn.cluster_id);
            m_writer.add_property("discarded", as_bool(turn.discarded));
            m_writer.add_property("has_colocated_both", as_bool(turn.has_colocated_both));
            m_writer.add_property("self_turn", as_bool(bg::detail::overlay::is_self_turn<bg::overlay_union>(turn)));
            // m_writer.add_property("visited", bg::visited_char(turn.operations[0].visited) + bg::visited_char(turn.operations[1].visited));
            m_writer.add_property("operation_0", lab(0));
            m_writer.add_property("operation_1", lab(1));
            m_writer.add_property("enriched_0", enr(0));
            m_writer.add_property("enriched_1", enr(1));
