...
#ifndef BOOST_GEOMETRY_ALGORITHMS_DETAIL_CLOSEST_POINTS_GEOMETRY_TO_SEGMENT_OR_BOX_HPP
#define BOOST_GEOMETRY_ALGORITHMS_DETAIL_CLOSEST_POINTS_GEOMETRY_TO_SEGMENT_OR_BOX_HPP
...
public:
    template <typename Segment, typename Strategies>
    ...
    {
        using comparable_return_type = 
            closest_points::creturn_t<Geometry, SegmentOrBox, Strategies>;
        ...
        {
            //TODO: compute and return the intersection    
        }
        ...
        auto comparable_distance = strategy::distance::services::get_comparable
            <
                decltype(strategies.distance(geometry, segment_or_box))
            >::apply(strategies.distance(geometry, segment_or_box));
        ...
        if (cd_min1 < cd_min2)
        {
            auto closest_point = strategies.closest_points(*pit_min, *segments_begin(geometry))
                .apply(*pit_min, *it_min1, *it_min2);

            set_segment_from_points::apply(*pit_min, closest_point, shortest_seg); 
            return;
        }
        else
        {
                ...
                <
                    ...
                    typename std::iterator_traits
                        <
                            segment_iterator_type
                        >::value_type
                >::apply(*it_min, *sit_min, shortest_seg, strategies);
        }
    }

    template <typename Segment, typename Strategies>
    static inline void apply(SegmentOrBox const& segment_or_box, Geometry const& geometry, 
...
template <typename MultiPoint, typename SegmentOrBox>
class geometry_to_segment_or_box
    <
        MultiPoint, SegmentOrBox, multi_point_tag
    >
{
private:
...
public:
    ...
    static inline void apply(MultiPoint const& multipoint,
                             ...
                             Strategies const& strategies)
    {
        closest_points::creturn_t<MultiPoint, SegmentOrBox, Strategies> cd_min;

        auto comparable_distance = strategy::distance::services::get_comparable
            <
                decltype(strategies.distance(multipoint, segment_or_box))
        ...
        dispatch::closest_points
            <
                ...
                SegmentOrBox
            >::apply(*it_min, segment_or_box, shortest_seg, strategies);
    }
};
...
#endif // BOOST_GEOMETRY_ALGORITHMS_DETAIL_CLOSEST_POINTS_GEOMETRY_TO_SEGMENT_OR_BOX_HPP
