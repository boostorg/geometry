...
struct buffer_range
{
                                ...
                                Strategies const& strategies)
    {
        ...
        switch (join)
        {
            ...
            case geometry::strategy::buffer::join_spike:
            {
                ...
                // duplicates
                std::vector<output_point_type> range_out;
                end_strategy.apply(penultimate_input,
                                   prev_perp2,
                                   previous_input,
                                   perp1,
                                   side,
            ...
            case geometry::strategy::buffer::join_convex:
            {
                ...
                if (line_line_intersection::apply(prev_perp1,
                                                  prev_perp2,
                                                  perp1,
                                                  perp2,
                                                  previous_input,
                                                  ...
                                                  intersection_point))
                {
                    ...
                    {
                        collection.add_piece(
                            geometry::strategy::buffer::buffered_join, previous_input, range_out);
                    }
                }
            }
            break;
    ...
    static inline geometry::strategy::buffer::join_selector
    get_join_type(output_point_type const& p0,
                  output_point_type const& p1,
                  output_point_type const& p2,
                  Strategies const& strategies)
    {
    ...
    template <typename Collection,
              ...
              typename EndStrategy,
              typename Strategies>
    static inline geometry::strategy::buffer::result_code
            ...
            output_point_type& last_p2)
    {
        ...
        point_type second_point, penultimate_point,
            ultimate_point; // last two points from begin/end
        ...
        geometry::strategy::buffer::result_code result
            = geometry::strategy::buffer::result_no_output;
        bool first = true;
        ...
        for (Iterator prev = it++; it != end; ++it)
        {
            ...
            geometry::strategy::buffer::result_code error_code
                = segment_strategy.apply(*prev, *it, side, distance_strategy, generated_side);
            ...
            collection.add_side_piece(
                *prev, *it, generated_side, first, distance_strategy.empty(side));
...
template <typename Multi, typename PolygonOutput, typename Policy>
struct buffer_multi
{
    template <typename Collection,
              typename DistanceStrategy,
...
template <typename OutputPointType,
          ...
          typename DistanceStrategy,
          typename PointStrategy>
inline void buffer_point(Point const& point,
                         Collection& collection,
                         DistanceStrategy const& distance_strategy,
                         PointStrategy const& point_strategy)
{
...
struct buffer_inserter<point_tag, Point, RingOutput>
{
    ...
    static inline void apply(Point const& point,
                             ...
                             Strategies const&)
    {
        detail::buffer::buffer_point<point_type_t<RingOutput>>(
...
template <typename RingInput, typename RingOutput>
struct buffer_inserter_ring
{
            ...
            Strategies const& strategies)
    {
        ...
        geometry::strategy::buffer::result_code result = buffer_range::iterate(collection,
                                                                               begin,
                                                                               end,
                                                                               side,
                                                                               distance_strategy,
          ...
          Strategies const& strategies)
    {
        ...
        using simplified_ring_t = model::ring<output_point_type,
        ...
        std::size_t const min_points = core_detail::closure::minimum_ring_size<
            geometry::closure<simplified_ring_t>::value>::value;
        ...
        if (code == geometry::strategy::buffer::result_no_output && n >= 1)
        {
            ...
            detail::buffer::buffer_point<output_point_type>(
                geometry::range::front(simplified), collection, distance, point_strategy);
        }
        return code;
    }
};
...
struct buffer_inserter<linestring_tag, Linestring, Polygon>
{
            ...
            output_point_type& first_p1)
    {
        ...
        else
        {
            ...
            geometry::strategy::buffer::result_code code
                = segment_strategy.apply(ultimate_point,
                                         penultimate_point,
                                         geometry::strategy::buffer::buffer_side_right,
                                         distance_strategy,
                                         generated_side);
    ...
    template <typename Collection,
              ...
              typename Strategies>
    static inline geometry::strategy::buffer::result_code
    apply(Linestring const& linestring,
          Collection& collection,
          DistanceStrategy const& distance,
          SegmentStrategy const& segment_strategy,
          JoinStrategy const& join_strategy,
          ...
          Strategies const& strategies)
    {
        ...
        if (n > 1)
        {
            ...
            output_point_type first_p1;
            code = iterate(collection,
                           boost::begin(simplified),
                           boost::end(simplified),
                           geometry::strategy::buffer::buffer_side_left,
                           distance,
                           segment_strategy,
                           join_strategy,
            ...
            {
                code = iterate(collection,
                               boost::rbegin(simplified),
                               boost::rend(simplified),
                               geometry::strategy::buffer::buffer_side_right,
                               distance,
                               segment_strategy,
                               join_strategy,
...
private:
    ...
    template <typename Iterator,
              ...
              typename PointStrategy,
              typename Strategies>
    static inline void iterate(Iterator begin,
                               Iterator end,
                               Collection& collection,
                               DistanceStrategy const& distance,
                               SegmentStrategy const& segment_strategy,
                               ...
                               bool is_interior)
    {
        for (Iterator it = begin; it != end; ++it)
        {
            ...
            bool const deflate = is_interior ? ! distance.negative() : distance.negative();

            collection.start_new_ring(deflate);
            geometry::strategy::buffer::result_code const code = policy::apply(*it,
                                                                               collection,
                                                                               distance,
                                                                               segment_strategy,
                                                                               join_strategy,
    ...
    template <typename InteriorRings,
              ...
              typename PointStrategy,
              typename Strategies>
    static inline void apply_interior_rings(InteriorRings const& interior_rings,
                                            Collection& collection,
                                            DistanceStrategy const& distance,
                                            SegmentStrategy const& segment_strategy,
                                            JoinStrategy const& join_strategy,
                                            ...
                                            Strategies const& strategies)
    {
        iterate(boost::begin(interior_rings),
    ...
    {
        {
            ...
            geometry::strategy::buffer::result_code const code
                = policy::apply(exterior_ring(polygon),
                                collection,
                                distance,
                                segment_strategy,
                                join_strategy,
            ...
            collection.finish_ring(
                code, exterior_ring(polygon), false, geometry::num_interior_rings(polygon) > 0u);
        }
...
template <typename Multi, typename PolygonOutput>
...
inline void buffer_inserter(GeometryInput const& geometry_input,
                            OutputIterator out,
                            DistanceStrategy const& distance_strategy,
                            SegmentStrategy const& segment_strategy,
                            JoinStrategy const& join_strategy,
                            ...
                            VisitPiecesPolicy& visit_pieces_policy)
{
    ...
    using collection_type
        = detail::buffer::buffered_piece_collection<geometry::ring_type_t<GeometryOutput>,
                                                    Strategies,
                                                    DistanceStrategy>;
    ...
    dispatch::buffer_inserter<tag_cast_t<tag_t<GeometryInput>, multi_tag>,
                              GeometryInput,
                              GeometryOutput>::apply(geometry_input,
                                                     collection,
                                                     distance_strategy,
                                                     segment_strategy,
                                                     join_strategy,
                            ...
                            Strategies const& strategies)
{
    ...
    buffer_inserter<GeometryOutput>(geometry_input,
                                    out,
                                    distance_strategy,
                                    segment_strategy,
                                    join_strategy,
