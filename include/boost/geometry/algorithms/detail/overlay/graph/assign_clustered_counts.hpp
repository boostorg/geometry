...
struct clustered_count_handler
{
    ...
    bool get_segment_points(Operation const& op, point_type const& point_turn, point_type& point_from, point_type& point_to)
    {
    ...
    void assign_ranks(point_type const& point_turn, std::vector<connection_item<point_type>>& item_vector)
    {
        std::size_t rank = 0;
        ...
        for (std::size_t i = 0; i + 1 < item_vector.size(); i++)
        {
            ...
            int const side = m_side_strategy.apply(point_turn, previous.properties.point, item.properties.point);
            item.properties.rank = side == 0 ? rank : ++rank;
        }
    }

    auto get_zone_counts(std::vector<connection_item<point_type>> const& item_vector, std::size_t rank_size)
    {
    ...
    void handle_spikes(cluster_info& cluster, std::vector<connection_item<point_type>>& item_vector)
    {
        for (std::size_t i = 0; i < item_vector.size(); i++)
        {
            ...
            auto const dm = get_distance_measure(current.opposite_point, current.point, next.point, m_intersection_strategy);
            if (dm.measure <= 0)
            {
                continue;
            }
    ...
    void assign_turn_operations(cluster_info const& cluster, connection_map_type const& connection_map)
    {
