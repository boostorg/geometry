/*************************************************************************
    > File Name: testcase_begin_equator_flat.cpp
    > Author: Rylynnn
    > Mail: jingry0321@gmail.com
    > Created Time: 2017年07月13日 星期四 00时51分31秒
 ************************************************************************/
#include <ctime>
#include <cmath>
...
int main()
{
    ...
    freopen("tc_begin_equator_flat.in", "r", stdin);
    freopen("tc_begin_equator_flat.out", "w", stdout);
    while(std::cin >> lon1 >> lat1 >> lon2 >> lat2 >> lon3 >> lat3 >> lon4 >> lat4){
        ...
        if (lon1 == lon3 && lat1 == lat3)
        {
            ...
            for (int i=0; i<MAX; i++)
            {
                cg_distance_result = bg::formula::flat_approximation<double>::apply(P1, P2, P4, earth); 
            }
            ...
            if(distance_result == cg_distance_result){
                 std::cout << "comparable_geographic_distance(Flat_approximation):" 
