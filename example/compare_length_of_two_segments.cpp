											   ...
											   point_type p4)
{
	...
	R1 = earth_r * (1 - earth_e2) / bg::math::sqrt((1 - earth_e2 * bg::math::sqr(sin(lat1)))
...
int main()
{
	...
	while(std::cin >> lon1 >> lat1 >> lon2 >> lat2 >> lon3 >> lat3 >> lon4 >> lat4){
		...
		std::cout << "comparable_geographic_distance (sec):" << secs_comparable_geographic_distance << std::endl << std::endl;
	}
